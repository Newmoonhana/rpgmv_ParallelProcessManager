 ...
 * 주요 개선 사항<br>
   알만툴 시리즈에는 병렬 이벤트를 여러개 만들 시 or (이벤트 한개더라도) 너무 많은 이벤트 처리할 시 게임이 렉이 걸리는 현상이 존재.<br>
   ->대충 1프레임 내로 너무 많은 작업을 하려고 해서 일어나는 프레임 멈춤 현상(좀 다르긴함. 이해 측면에서 설명한거)<br>
   이는 사실 다른 제작툴에서도 존재할 수 있으나, 알만툴 특유의 보이지 않는 대기 프레임으로 인해 알만툴의 큰 단점처럼 느껴짐.<br>
   1) 이 문제를 완화시키고자 해당 플러그인을 개발했습니다. 물론 특정 프레임에서 너무 많은 이벤트가 호출된다면 렉 걸리는 문제 자체가 해결된 건 아니지만, 사용자의 역량에 따라 더 완화시킬 수 있습니다.<br>
   2) 병렬 이벤트 내의 불필요하게 자추 호출되는 코드들을 덜 호출되도록 합니다. 굳이 1프레임마다 호출할 필요가 없는 이벤트는 조건 분기로 특정 프레임에만 호출될 수 있도록 합니다.<br>
   ( 해당 최적화 기법에 대해 설명하는 영상: https://youtube.com/shorts/BuvVXUpVgfI?si=nbtvSH_-41Wl5AAa )<br>
   3) 스파게티 코드를 방지할 수 있습니다. 다만 여기부터는 사용자 실력에 따라 편차가 좀 갈릴 것으로 보입니다.<br>
